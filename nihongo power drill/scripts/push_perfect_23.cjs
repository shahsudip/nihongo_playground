...
async function pushPerfect23() {
  const jsonPath = path.join(__dirname, 'grammar_23_perfect.json');
  const chapterData = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));

  const chapterDocRef = db.collection('books').doc('nihongo-power-drill-n3').collection('chapters').doc('grammar-23');
  ...
  const existingIdx = mainDb.chapters.findIndex(c => c.id === 'grammar-23');
  if (existingIdx !== -1) {
  ...
  console.log('Successfully updated local power_drill_data.json with Drill 23');
}
