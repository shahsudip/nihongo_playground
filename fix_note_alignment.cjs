...
async function fixNoteAlignment() {
  console.log("Fixing note alignment...");
  ...
  skm.chapters.forEach(c => {
    c.passages.forEach(p => {
      ...
      if (p.passageText) {
        ...
        p.passageText = p.passageText.replace(/class="text-sm opacity-80 font-normal"/g, 'class="text-[0.8em] align-super opacity-80 font-normal"');
