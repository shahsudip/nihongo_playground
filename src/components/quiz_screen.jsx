...
const Quiz = ({ quizContent, quizTitle, quizType, onComplete, onEndQuizEarly }) => {
  ...
  const handleContinueReview = () => { setShowFirstPassModal(false); acknowledgeFirstPass(); };
...
const QuizPage = () => {
  ...
  // --- THIS IS THE FIX ---
  // The quizId is now created in one single, reliable place using useMemo.
  const quizId = useMemo(() => {
    if (!isCustomQuiz && !selectedDifficulty) return null; // Return null if no difficulty is chosen yet
    return isCustomQuiz ? level : `${level}-${category}-${selectedDifficulty}`;
  }, [isCustomQuiz, level, category, selectedDifficulty]);

  // This useEffect now depends on the stable quizId.
  useEffect(() => {
    if (quizId) { // Only run if we have a valid quizId
  ...
  }, [quizId]); // This effect re-runs only when the quizId changes
  ...
  // This function now uses the stable quizId from the useMemo hook.
  const onQuizComplete = (finalScoreValue, totalAttempts) => {
    if (!isCustomQuiz) {
      const storageKey = `quiz-progress-${level}-${category}`;
      if (selectedDifficulty === 'easy' && unlockedDifficulty === 'easy') localStorage.setItem(storageKey, 'medium');
      else if (selectedDifficulty === 'medium' && unlockedDifficulty === 'medium') localStorage.setItem(storageKey, 'hard');
    }
  ...
  const handleModalContinue = () => { setShowCompletionModal(false); navigate('/profile'); };
  const handleEndQuizEarly = (stats) => { navigate('/results', { state: { score: stats.score, total: stats.total, quizId: quizId } }); };

  const renderDifficultySelection = () => { /* ... Unchanged ... */ };
  const renderQuiz = () => {
    ...
    return <Quiz key={quizId} quizContent={quizContent} quizTitle={quizTitle} quizType={category} onComplete={onQuizComplete} onEndQuizEarly={handleEndQuizEarly} />;
  };
