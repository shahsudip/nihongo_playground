...
async function updateNoteStyles() {
  console.log("Updating note styles...");
  ...
  skm.chapters.forEach(c => {
    c.passages.forEach(p => {
      ...
      if (p.passageText) {
        // Find all instances of the old class and replace them
        p.passageText = p.passageText.replace(/class="text-xs text-gray-500 font-normal"/g, 'class="text-sm opacity-80 font-normal"');
        
        if (before !== p.passageText) {
          changedChapters.add(c);
          console.log(`Updated note styles in: ${p.title}`);
        }
      }
    });
  });
