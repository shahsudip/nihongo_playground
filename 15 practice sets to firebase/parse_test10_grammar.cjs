...
const raw = fs.readFileSync('test10_grammar_paste.txt', 'utf8');
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (l.includes('食べ物とコミュニケーション') || l.includes('「男性の心をつかむには') || l.includes('この言葉は') || l.includes('たとえば') || l.includes('実際に') || l.includes('食事が重要な役割')) {
        readingPassage += l + "\n";
        continue;
    }
...
if (qs.length === 23) {
    ...
    dbData.sets[9].sections['grammar-reading'].questions = qs;
    ...
    console.log('Successfully updated Test 10 Grammar in database!');
} else {
