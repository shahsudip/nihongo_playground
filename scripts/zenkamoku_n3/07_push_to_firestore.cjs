 ...
 * Push all zenkamoku_n3 JSON chapters to Firestore.
 * Collection: books/zenkamoku-n3-best-workbook/chapters/{chapterId}
 */
const fs = require('fs');
...
const BOOK_ID = 'zenkamoku-n3-best-workbook';
const DATA_DIR = path.join(__dirname, '..', '..', 'src', 'data', 'zenkamoku_n3');
...
async function push() {
  ...
  await db.collection('books').doc(BOOK_ID).set({
    ...
    titleEn: 'The Best Complete Workbook for JLPT N3',
    level: 'N3',
